...
func cmdChat(cfg config.Config, args []string) error {
	...
	fmt.Fprintln(os.Stderr, ui.Dim("1trade chat — type a message; /help for commands, /exit to quit"))
	for {
...
// saveTranscript writes the conversation as plain "role: content" lines (default 1trade-chat.txt).
func saveTranscript(path string, history []map[string]string) error {
	if path == "" {
		path = "1trade-chat.txt"
	}
...
func fetchModelMeta(cfg config.Config) map[string]modelMeta {
	...
	for _, m := range resp.Data {
		out[m.ID] = modelMeta{price: parseFloat(m.Trade1.Price), creditType: m.Trade1.CreditType}
	}
	return out
}
