...
func requireToken(cfg config.Config) error {
	if cfg.Token == "" {
		return errors.New("not logged in — run: 1trade login")
	}
	return nil
}
...
func cmdLogin(cfg config.Config, args []string) error {
	...
	password := fs.String("password", "", "account password (prefer the prompt or TRADE1_PASSWORD)")
	_ = fs.Parse(args)
	e := firstNonEmpty(*email)
	if e == "" {
		e = readLine("Email: ")
	}
	p := firstNonEmpty(*password, os.Getenv("TRADE1_PASSWORD"))
	if p == "" {
...
func cmdCredits(cfg config.Config, args []string) error {
	if len(args) == 0 {
		return errors.New("usage: 1trade credits balance|transactions|convert")
	}
	switch args[0] {
	case "balance":
...
func creditsConvert(cfg config.Config, args []string) error {
	...
	_ = fs.Parse(args)
	if *from == "" || *to == "" || *amount == "" {
		return errors.New("usage: 1trade credits convert --from ai_index --to text --amount 100")
	}
	var out struct {
...
func cmdInfer(cfg config.Config, args []string) error {
	if len(args) == 0 || args[0] != "chat" {
		return errors.New(`usage: 1trade infer chat -m MODEL "prompt"`)
	}
	if err := requireToken(cfg); err != nil {
		return err
	}
	...
	if text == "" {
		return errors.New(`provide a prompt: 1trade infer chat -m llama-3.1-8b "hello"`)
	}
...
func cmdKeys(cfg config.Config, args []string) error {
	...
	if len(args) == 0 {
		return errors.New("usage: 1trade keys create|list|revoke")
	}
	switch args[0] {
	case "create":
		...
		_ = fs.Parse(args[1:])
		if *name == "" {
			return errors.New("usage: 1trade keys create --name production")
		}
	...
	case "revoke":
		...
		if len(rest) < 1 {
			return errors.New("usage: 1trade keys revoke <id> [--yes]")
		}
...
func cmdConfig(cfg config.Config, args []string) error {
	...
	if args[0] == "set" {
		if len(args) < 3 {
			return errors.New("usage: 1trade config set <platform_url|gateway_url|ledger_url|compute_url> <value>")
		}
		switch args[1] {
		case "platform_url":
	...
	return errors.New("usage: 1trade config get|set")
}
