...
func cmdGPU(cfg config.Config, args []string) error {
	if len(args) == 0 {
		return errors.New("usage: 1trade gpu types|create|list|get|stop|start|delete")
	}
	switch args[0] {
	case "types":
		return gpuTypes(cfg)
...
func gpuCreate(cfg config.Config, args []string) error {
	...
	_ = fs.Parse(args)
	if *gtype == "" {
		return errors.New("usage: 1trade gpu create --type h100 [--count N] [--image stable] [--region R]")
	}
...
func gpuGet(cfg config.Config, args []string) error {
	...
	if len(args) < 1 {
		return errors.New("usage: 1trade gpu get <id>")
	}
	var inst instance
...
func gpuAction(cfg config.Config, action string, args []string) error {
	...
	if len(args) < 1 {
		return fmt.Errorf("usage: 1trade gpu %s <id>", action)
	}
	var inst instance
...
func gpuDelete(cfg config.Config, args []string) error {
	...
	if len(rest) < 1 {
		return errors.New("usage: 1trade gpu delete <id> [--yes]")
	}
