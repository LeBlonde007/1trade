...
const textModels = computed(() => models.value.filter((m) => m.trade1.modality === 'text'))
...
const estCost = computed(() => {
  ...
  return ((Number(activeModel.value.trade1.price) * result.value.usage.total_tokens) / 1000).toFixed(6)
})
        ...
        <section class="panel">
          <header class="panel-h">
            ...
            <select v-model="selectedModel" class="mini-select mono">
              <option v-for="m in textModels" :key="m.id" :value="m.id">{{ m.id }} · {{ fmt(m.trade1.price) }}/{{ m.trade1.unit }}</option>
            </select>
          </header>
          <div class="panel-b">
