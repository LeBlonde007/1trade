...
| F04 | `exascale` CLI (login → infer → gpu → billing) | Platform | M1→M6 | 🔵 | ~40% | **v0 complete (v0.1.4).** login/whoami/credits balance+convert/catalog/infer chat/keys/config — Go client over the live APIs. **Proven live: login → convert → infer chat.** M3+: gpu/cluster/train/billing as F12/F13 land; distribution (brew/apt/pip) M6. |
