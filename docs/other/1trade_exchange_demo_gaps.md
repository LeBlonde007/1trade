# 1Trade — Exchange Demo Gaps

> Audited 2026-05-24 against `/mnt/f/ex/1Trade Frontend/app/pages/**` and the
> three personas in `1trade_persona_demo_flows.md`. Lists every step of the
> end-to-end exchange demo and the screens that are **still missing** to make
> that full loop runnable on the frontend.
>
> Saved here instead of overwriting `1trade_mvp_screens_prompts.md` (the
...
- **Step 3 — Instructions:**
  - Wire: reveal bank coordinates with copy buttons (Bank name · Routing · Account · Beneficiary `1Trade FBO Jordan Park` · **Reference code** `EXA-DEP-9F2A` mono — call out *"include the reference or your wire will sit"*).
...
- **Step 2 — Install the 1Trade agent:**
...
5. **D1 → D3 → D4** — supply side (demonstrates 1Trade is two-sided).
