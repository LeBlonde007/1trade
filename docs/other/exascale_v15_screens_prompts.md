# Exascale — v1.5 Screens & UIs (post-MVP prompts catalog)

> Companion to `exascale_mvp_screens_prompts.md` (the original 26-prompt MVP set)
> and `exascale_mvp_screens_checklist.md` (status tracker — currently 24 / 26 done).
>
...
>
> ### How to use
> 1. Open a fresh chat in the Exascale claude.ai project (it has A1–A4
...
- [ ] **H3. SSO / SAML configuration wizard** (`/enterprise/sso`)
  ...
  STEPS:
    ...
    2. Exchange metadata — download Exascale SP metadata, paste IdP metadata XML or URL
...
- [ ] **J3. Status page** (`/status`)
  MODE: Light, public, scoped to Exascale's own systems.
  CONTENT
...
## Tier K — Admin (internal Exascale ops, dark)
