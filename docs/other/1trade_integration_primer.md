# 1Trade — Hardware & Datacenter Integration Primer

> Pairs with `1trade_beginner_primer.md`. That one taught you the
...
The unit of capacity 1Trade trades. **One GPU-hour = one H100 (or
...
- **Tier I-IV** — Uptime Institute's redundancy rating. Tier III = no
  ...
  downtime (dual everything). 1Trade requires Tier III minimum.
...
## 3. The integration — how a datacenter plugs into 1Trade
...
`docs/1trade_exchange_demo_gaps.md`.
...
- The DC's host *is* the identity, not a user-typeable secret that
  can leak. Cert lives on disk in `/etc/1trade/agent.crt`.
