# Exascale MVP Screens — Claude Design Prompts
...
2. **Have the design brief open** (`exascale_design_system_brief.md`) — you'll paste the foundation tokens from it first
...
```
DESIGN TOKENS — Exascale v1 (placeholder, swap when brand book lands)
...
DESIGN PRINCIPLES
  ...
  3. BOLDNESS COMES FROM PRECISION, NOT MAXIMALISM
     The frontend-design skill says be bold. For Exascale, boldness
...
```
Design the Exascale public homepage as a single HTML artifact.

CONTEXT
Exascale is the commodity market for AI compute — a three-sided market 
...
1. TOP NAV — sticky on scroll, subtle backdrop blur when scrolled past hero
   - Left: "Exascale" wordmark (use a tight modern sans, plain text — no logo yet)
...
2. HERO — the focal point
   ...
   - LIVE INDEX TICKER — the killer feature
     ...
     "Exascale AI Index · $1 = 1,002.4 AI credits · ▲ 0.18% (24h)"
...
4. THE THREE-SIDED MARKET — diagram + explanation
   ...
   - Custom SVG diagram showing three nodes connected by arrows to a 
     central "Exascale Trading Venue" hub:
       [ GPU DATACENTERS ] → [ TRADERS ] → [ AI COMPANIES ]
            Supply             Liquidity        Demand
     All converging into a central rectangle labeled "Exascale Venue."
...
10. FOOTER
    ...
    - Bottom row: Exascale wordmark, copyright, status indicator 
...
```
Design the Exascale trading dashboard — the HEADLINE product screen.

CONTEXT
This is the single most important visual asset Exascale will produce 
...
whether Exascale is a credible market venue.
...
TOP BAR — 60px tall
  Left:
    - Exascale wordmark (small, monochrome)
...
```
Design a single-market detail page for the Exascale platform.
...
5. MARKET DETAILS — two columns
   LEFT COLUMN — Description and metadata
     ...
     - 2-3 paragraphs explaining what EAI-IDX is:
       "The Exascale AI Index represents the unified price of AI 
...
```
Design the Exascale credit wallet — where users see all their credit 
balances and convert between them.

CONTEXT
Users hold three classes of asset on Exascale:
  - Cash (USD or JPY)
...
```
Design the Exascale AI Index methodology page — the credibility document.

CONTEXT
The Exascale AI Index is the headline tradeable instrument and the 
...
CONTENT AREA — body 720px reading column
  ...
  HEADER
    Eyebrow: "— METHODOLOGY"
    Title: "Exascale AI Index Methodology" h1
  ...
  SECTION 1: EXECUTIVE SUMMARY (~150 words)
    "The Exascale AI Index is a daily reference price for AI inference,
  ...
  FOOTER OF PAGE
    Contact: "methodology@exascale.com · audit@exascale.com"
...
```
Design the Exascale signup page.
...
LEFT — FORM
  Heading "Open an Exascale account"
...
```
Design the Exascale portfolio overview page.
...
```
Design the Exascale trade history page.
...
```
Design the Buy Credits flow — how users add cash to Exascale and convert it to credits.
...
onboarded by Exascale's enterprise sales team or onboarding the 
...
2. PAGE HEADER
   ...
   Right: 
     ...
     [CLI command] code-styled inline showing "exascale gpu create"
...
6. EMPTY STATE (also visible)
   ...
   Two paths:
     [Via web] button
     [Via CLI] code block: "exascale gpu create --type h100 --count 8"
...
Modal or page (treat as page).
  ...
  HEADER
    ...
    Subhead: "Or use the CLI: exascale gpu create"
  ...
  COLUMN 2 — Configuration
    Section: Base image
      Dropdown:
        Exascale ML Stack (default) — PyTorch + CUDA pre-installed
...
RIGHT — PLAYGROUND
  ...
  Chat-style conversation area:
    Mock conversation showing:
      ...
      trading in Exascale's v1 architecture."
...
```
Design the Exascale login page.
...
LEFT — minimal form
  ...
  Below:
    ...
    "New to Exascale? Open an account →" link
...
CONTEXT
Internal-only in v1 — used by Exascale ops + DC partners. Shows 
...
4. REVENUE THIS MONTH
   ...
   Below: 
     ...
     Exascale fee (30%): $370,350
     Next payout: 2026-06-01

5. PAYOUT HISTORY table
   Recent payouts:
     Period | Capacity sold | Gross | Exascale fee | Your payout | Status
...
CONTEXT
Used by Exascale ops team in v1 to onboard new DC partners. 
Self-serve in v1.5+.
...
1. Section header: "API Keys"
   Subhead: "Programmatic access to Exascale APIs"
...
CONTENT
  ...
  NOTIFICATION LIST:
    ...
    Mock examples:
      ...
      ○ 2d ago — "Welcome to Exascale! Take the 60-second tour →"
...
6. ERROR: NETWORK CONNECTION LOST
   Sticky banner at top of page:
     ...
     "Connection to Exascale lost. Reconnecting..."
