...
A token from OpenAI costs whatever they say it costs today. 1Trade makes
...
| **Index** | A weighted basket — 1Trade's `EAI-IDX` blends all the major credit families into one number, like the S&P 500. |
...
1. Create `app/pages/hello.vue`:
   ...
   useHead({ title: 'Hello — 1Trade' })

   const greeting = ref('Welcome to 1Trade.')
   </script>
   ...
   ```
2. Start the dev server: `cd "/mnt/f/ex/1Trade Frontend" && npm run dev`
...
|---|---|
| Understand the product strategy | `docs/1trade_persona_demo_flows.md` |
| Find every screen and its status | `docs/1trade_mvp_screens_checklist.md` |
| Get prompts to design a new screen | `docs/1trade_mvp_screens_prompts.md` (MVP) + `docs/1trade_v15_screens_prompts.md` (v1.5) |
| See what's missing from the full exchange demo | `docs/1trade_exchange_demo_gaps.md` |
